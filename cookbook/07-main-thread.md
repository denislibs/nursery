...
прошло больше бюджета, он делает `await yieldToMain()`. Браузер успевает обработать клик и
отрисовать кадр.

Бюджет по умолчанию `'auto'`: половина интервала кадра, который измеряется по меткам
`requestAnimationFrame` и кэшируется на несколько секунд. На 60 Hz это около 8 мс, на 120 Hz
около 4 мс, поэтому цикл не съедает кадр на быстрых экранах. В фоне или без `rAF` берётся
60 Hz. Явное число в `budget` отключает измерение. В Chromium `chunked` ещё смотрит на
`navigator.scheduling.isInputPending()` и уступает сразу, если пользователь уже что-то нажал,
не дожидаясь бюджета.

`frameInterval()` доступен отдельно, если нужен тот же замер для своей логики.
