...
describe('hooks', () => {
  test('onRequest can rewrite url and init before fetch', async () => {
    ...
    const http = createHttp({
      fetch: f.fn,
      onRequest: (url, init) => ({ url: url + '?traced=1', init: { ...init, headers: { ...headersOf(init), 'x-trace': 't1' } } }),
    });
  ...
  test('onRequest may be async and see the request signal', async () => {
    ...
    const http = createHttp({ fetch: f.fn, onRequest: async (_u, init) => { seen = init.signal!; } });
  ...
  test('onResponse can replace the response and observe status', async () => {
    ...
    const http = createHttp({
      fetch: f.fn,
      onResponse: res => { seen.push(res.status); return res.status === 500 ? json({ v: 'patched' }) : res; },
    });
  ...
  test('onResponse runs per attempt when retrying', async () => {
    ...
    const http = createHttp({ fetch: f.fn, retry: { retries: 1, delay: 0 }, onResponse: r => { statuses.push(r.status); return r; } });
...
describe('schema validation', () => {
  ...
  test('a throwing parser rejects with the parser error, response attached', async () => {
    ...
    const http = createHttp({ fetch: f.fn });
    const err = await http.get('https://x/u', { signal: sig(), parse: () => { throw new TypeError('invalid shape'); } }).catch((e: Error) => e);
...
describe('deadline', () => {
  test('per-request deadline shortens the attempt timeout', async () => {
    ...
    const expectation = expect(http.get('https://x/a', { signal: sig(), deadline: performance.now() + 200 }))
  ...
  test('an already-expired deadline rejects without fetching', async () => {
    const f = fakeFetch(() => json({}));
    const http = createHttp({ fetch: f.fn });
    await expect(http.get('https://x/a', { signal: sig(), deadline: performance.now() - 1 }))
      .rejects.toSatisfy((e: unknown) => (e as DOMException).name === 'TimeoutError');
...
describe('streams', () => {
  test('stream() yields NDJSON objects', async () => {
    const f = fakeFetch(() => new Response(textStream(['{"a":1}\n{"a":', '2}\n', '{"a":3}']), { headers: { 'content-type': 'application/x-ndjson' } }));
    const http = createHttp({ fetch: f.fn });
  ...
  test('sse() parses events with id, event and multi-line data, and sends Accept header', async () => {
    const f = fakeFetch(() => new Response(textStream([
      'id: 1\nevent: tick\ndata: {"n":1}\n\n',
      ': comment\n',
      'data: line1\ndata: line2\n\n',
      'retry: 5000\nid: 3\ndata: last\n\n',
  ...
  test('sse() without reconnect ends when the stream ends', async () => {
    const f = fakeFetch(() => new Response(textStream(['data: only\n\n']), { headers: { 'content-type': 'text/event-stream' } }));
    const http = createHttp({ fetch: f.fn });
    const events = await toArray(http.sse('https://x/events', { signal: sig() }));
