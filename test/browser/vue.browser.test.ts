...
describe('useScope', () => {
  test('scope closes when the component unmounts', async () => {
    ...
    mount(defineComponent({ setup() { signal = useScope().signal; return () => h('div'); } }));
...
describe('useScopedWatch', () => {
  test('re-runs with a fresh scope when a dependency changes, closing the old one', async () => {
    ...
    mount(defineComponent({ setup() { useScopedWatch(scope => { void id.value; signals.push(scope.signal); }); return () => h('div'); } }));
    id.value = 2;
    await nextTick();
...
describe('useAsync', () => {
  test('tracks loading/data and drops results of cancelled runs', async () => {
    ...
    mount(defineComponent({ setup() {
      state = useAsync(async scope => { const cur = id.value; await sleep(cur === 1 ? 50 : 5, scope.signal); return `user-${cur}`; });
      return () => h('div');
    } }));
...
describe('useEventStream', () => {
  test('subscribes when the element ref is set and stops on unmount', async () => {
    ...
    mount(defineComponent({ setup() {
      useEventStream<MouseEvent>(el, 'click', e => { seen.push(e.type); });
...
describe('useWorker', () => {
  test('exposes the remote api and terminates on unmount', async () => {
    ...
    mount(defineComponent({ setup() {
      remote = useWorker<typeof EchoApi>(() => new Worker(new URL('./fixtures/echo.worker.ts', import.meta.url), { type: 'module' }));
      return () => h('div');
    } }));
