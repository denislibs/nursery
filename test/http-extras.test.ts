...
function fakeFetch(handler: (url: string, init: RequestInit & { duplex?: string }) => Response | Promise<Response>) {
...
describe('query serialization', () => {
  test('nested objects use bracket notation and arrays repeat the key', async () => {
    ...
    await http.get('https://x/items', { signal: sig(), query: { filter: { a: 1, b: 'x' }, ids: [1, 2], skip: undefined, flag: true } });
  ...
  test('a custom querySerializer replaces the default', async () => {
    const f = fakeFetch(() => json({}));
    const http = createHttp({ fetch: f.fn, querySerializer: q => 'raw=' + encodeURIComponent(JSON.stringify(q)) });
...
describe('progress', () => {
  test('onDownloadProgress reports loaded/total while the body streams', async () => {
    ...
    const res = await http.request('https://x/file', { signal: sig(), onDownloadProgress: (loaded, total) => { seen.push([loaded, total]); } });
  ...
  test('onUploadProgress streams the body with duplex half and reports sent/total', async () => {
    ...
    await http.post('https://x/upload', { signal: sig(), body: payload, onUploadProgress: (sent, total) => { seen.push([sent, total]); } });
  ...
  test('onUploadProgress works for JSON bodies too', async () => {
    let text = '';
    const f = fakeFetch(async (_u, init) => {
      for await (const chunk of init.body as ReadableStream<Uint8Array>) text += new TextDecoder().decode(chunk);
      return json({});
    });
    const http = createHttp({ fetch: f.fn });
    const seen: number[] = [];
    await http.post('https://x/j', { signal: sig(), body: { hello: 'world' }, onUploadProgress: sent => { seen.push(sent); } });
