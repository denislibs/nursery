...
describe('postTask', () => {
  test('uses scheduler.postTask with the given priority and returns the result', async () => {
    ...
    vi.stubGlobal('scheduler', {
      postTask: (fn: () => unknown, opts: unknown) => { calls.push(opts); return Promise.resolve(fn()); },
    });
  ...
  test('forwards the signal to scheduler.postTask', async () => {
    let seen: unknown;
    vi.stubGlobal('scheduler', { postTask: (fn: () => unknown, opts: { signal?: AbortSignal }) => { seen = opts.signal; return Promise.resolve(fn()); } });
...
describe('Queue priority', () => {
  test('higher priority waiting tasks start first; equal priority stays FIFO', async () => {
    ...
    q.add(async () => { order.push('high-1'); }, { priority: 10 });
    q.add(async () => { order.push('default'); });                // default priority 0, after 'low'
    q.add(async () => { order.push('high-2'); }, { priority: 10 });
    release();
    await q.idle();
...
describe('on() typing', () => {
  test('infers the event type from the target event map', async () => {
    ...
    const el = { addEventListener: target.addEventListener.bind(target), removeEventListener: target.removeEventListener.bind(target), dispatchEvent: target.dispatchEvent.bind(target) } as unknown as HTMLButtonElement;
